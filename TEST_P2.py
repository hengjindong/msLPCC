...
def test(model_1, args, batch_size=1):
    ...
    # VAL_DATASET = MMDataset_with_Cylinder_total(root=args.dataset_path,
    ...
    for step, data in tqdm(enumerate(test_loader), total=(len(test_loader)), smoothing=0.9):
        with torch.no_grad():
            ...
            # cd3 = chamfer_distance(pc_gt_raw, decoder_output)[0]
            # EMD = emdModule()
            # dis, assigment = EMD(pc_gt_raw, decoder_output, 0.002, 10000)
            # emd = (dis.cpu().detach()).mean()
            ...
            d2_results = cal_d1(decoder_output, pc_gt_raw, step, checkpoint_path)
...
if __name__ == '__main__':
    ...
    # exp_name = '10000_256_2048'
    exp_name = '8001_256_2048'
    # model_name = 'A_PCT_PCC'
    model_name = 'A_PN_PCC'
    # model_name = 'A_NGS_PCC'
    # model_name = 'A_PNPP_PCC'
    # model_name = 'A_Trans_PCC'
