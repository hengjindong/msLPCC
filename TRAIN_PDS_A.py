...
def parse_args():
    ...
    parser.add_argument('--model', default='PCT_D_S_PCC_SR', help='model name')
    parser.add_argument('--epoch',  default=110, type=int, help='number of epoch in training [default: 100]')
    ...
    parser.add_argument('--lamb', default=8050, type=int)
...
def test(args, model, depth_enc_model, sem_enc_model, loader, criterion, global_epoch=None):
    mean_loss = []
    mean_bpp = []
    mean_cd = []
    with torch.no_grad():
        for j, data in tqdm(enumerate(loader, 0), total=(len(loader)),smoothing=0.9):
            ...
            with torch.no_grad():
                ...
                ft_sem = sem_enc_model(points)
                ft_d_s = torch.cat((ft_sem, ft_depth), dim = 1)

            bpp, recon_xyz, cd_ds, cd_raw = model(ft_d_s, points, points_raw)
...
def main(args):
    ...
    for epoch in tqdm(range(0,args.epoch)):
        ...
        if epoch%10==0:
            ...
            with torch.no_grad():
                mean_loss_test, mean_bpp_test, mean_cd_test = test(args, model.eval(), depth_enc_model, sem_enc_model, valDataLoader, criterion, global_step)
